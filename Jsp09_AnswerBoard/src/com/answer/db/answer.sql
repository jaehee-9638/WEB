...
--원댓글 
--글번호:NEXTVAL  ,그룹번호:NEXTVAL  ,그룹순서:1  , 제목공백:0


--1번글 작성
INSERT INTO ANSWERBOARD VALUES(BOARDNOSEQ.NEXTVAL,GROUPNOSEQ.NEXTVAL,1,0,
							'제목입니다','내용입니다','작성자입니다.',SYSDATE);
...
--엔써보드 업데이트 할거고 그룹순서를 원래있던 그룹순서에+1할거고 
--WHERE 조건은 그룹번호가 첫번째 괄호 두번째 괄호 둘다 맞아야함 (1.그룹번호 같아야함 , 2.그룹순서는 더 커야함)
...
--엔써보드 인서트 할거고 
--글번호 : 시퀀스에서 숫자 하나 만들고 
--그룹번호 : 글번호 1번호랑 같은 글번호로 올림 
--그룹순서 : 글번호 1번호에서 1더함
--제목공백 : 글번호 1번호에서 1더함

--2번글 작성 
INSERT INTO ANSWERBOARD 
VALUES (BOARDNOSEQ.NEXTVAL,GROUPNOSEQ.NEXTVAL,1,0,'2번글입니다.','2번 원글','앨리스',SYSDATE);
...
VALUES(
		...
		(SELECT TITLETAB FROM ANSWERBOARD WHERE BOARDNO=3) ,
		'RE:2번글 입니다.',
		'아이고 어렵다',
		'강사',
		SYSDATE
...
--2번글에 답변 다시하기 
--UPDATE 
--부모와 같은 그룹인 글들을 찾아서, 부모의 순서보다 큰 순서의 글들을 순서+1
UPDATE ANSWERBOARD 
SET GROUPSEQ =GROUPSEQ+1 
